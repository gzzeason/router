...
function getTailscaleBin() {
  try {
    const systemPath = execSync(
      "which tailscale 2>/dev/null || where tailscale 2>nul",
      { encoding: "utf8" },
    ).trim();
...
export function isTailscaleRunning() {
  ...
  try {
    const out = execSync(
      `"${bin}" ${SOCKET_FLAG.join(" ")} funnel status --json 2>/dev/null`,
      { encoding: "utf8" },
    );
    const json = JSON.parse(out);
...
async function installTailscaleMac(sudoPassword, log) {
  ...
  await new Promise((resolve, reject) => {
    ...
    child.on("close", (c) => {
      try {
        execSync(`rm -f ${pkgPath}`, { stdio: "ignore" });
      } catch {
        /* ignore */
      }
      if (c === 0) resolve();
...
export async function startDaemonWithPassword(sudoPassword) {
  ...
  if (!fs.existsSync(TAILSCALE_DIR))
...
export async function startFunnel(port) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const parseFunnelUrl = (text) =>
      (text.match(/https:\/\/[a-z0-9-]+\.[a-z0-9.-]+\.ts\.net[^\s]*/i) ||
        [])[0]?.replace(/\/$/, "") || null;
...
export async function stopDaemon(sudoPassword) {
  // Try non-sudo first
  try {
    execSync("pkill -x tailscaled", { stdio: "ignore", timeout: 3000 });
  } catch {
    /* ignore */
  }
