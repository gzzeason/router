    ...
    <div className="flex flex-col gap-8">
      ...
      {isCompatible && providerNode && (
        <Card>
          ...
          {connections.length > 0 && (
            <p className="text-sm text-text-muted">
              Only one connection is allowed per compatible node. Add another
              node if you need more connections.
            </p>
          )}
        </Card>
      )}
...
function ModelRow({
  ...
  return (
    <div
      className={`group px-3 py-2 rounded-lg border ${borderColor} hover:bg-sidebar/50`}
    >
      <div className="flex items-center gap-2">
        <span
        ...
        <div className="flex flex-col gap-1">
          ...
          {model.name && (
            <span className="text-[9px] text-text-muted/70 italic pl-1">
              {model.name}
            </span>
          )}
        </div>
...
function CooldownTimer({ until }) {
  ...
  return (
    <span className="text-xs text-orange-500 font-mono">⏱ {remaining}</span>
  );
}
...
function ConnectionRow({
  ...
  return (
    <div
      className={`group flex items-center justify-between p-2 rounded-lg hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors ${connection.isActive === false ? "opacity-60" : ""}`}
    >
