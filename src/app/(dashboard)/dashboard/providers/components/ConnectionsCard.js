...
function ConnectionRow({
  ...
  return (
    <div
      className={`group flex items-center justify-between p2 rounded-lg hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors ${connection.isActive === false ? "opacity-60" : ""}`}
    >
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await onSave({
        ...
        proxyPoolId:
          formData.proxyPoolId === NONE ? null : formData.proxyPoolId,
          ...
          <div className="flex items-center gap-2">
            ...
            <Toggle
              ...
              onChange={(enabled) => {
                ...
                if (enabled && !providerStickyLimit)
                  setProviderStickyLimit("1");
                saveStrategy(
                  strategy,
              ...
              {connections.map((conn, idx) => (
                <ConnectionRow
                  ...
                  onToggleActive={(isActive) =>
                    handleToggleActive(conn.id, isActive)
                  }
