  ...
  const handleApply = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage({
          type: "success",
          text: data.message || "Settings applied successfully!",
        });
        checkStatus();
      } else {
        setMessage({
                ...
                <div className="flex flex-col gap-1">
                  ...
                  {modelList.length > 0 && (
                    <div className="flex flex-col gap-1 mb-1">
                      {modelList.map((id) => (
                        <div
                          key={id}
                          className="flex items-center gap-2 px-3 py-1.5 bg-bg-secondary rounded-lg border border-border"
                        >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className={`px-3 py-2 rounded-lg border text-sm transition-colors shrink-0 ${activeProviders?.length ? "bg-bg-secondary border-border hover:border-primary cursor-pointer" : "opacity-50 cursor-not-allowed border-border"}`}
                    >
                      Select
                    </button>
                    <button
                      ...
                      className="px-3 py-2 rounded-lg border text-sm bg-bg-secondary border-border hover:border-primary transition-colors shrink-0 disabled:opacity-50"
                      title="Add model"
                    >
