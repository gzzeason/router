<div align="center">
  <img src="../images/router.png?1" alt="Router Dashboard" width="800"/>
  ...
  **OpenClaw 的免费 AI 提供商。**
  ...
  [![npm](https://img.shields.io/npm/v/router.svg)](https://www.npmjs.com/package/router)
  [![Downloads](https://img.shields.io/npm/dm/router.svg)](https://www.npmjs.com/package/router)
  [![License](https://img.shields.io/npm/l/router.svg)](https://github.com/decolua/router/blob/main/LICENSE)
  
  [🚀 快速开始](#-quick-start) • [💡 特性](#-key-features) • [📖 设置](#-setup) • [🌐 网站](https://router.com)
</div>
...
此仓库包是私有的（`router-app`），因此源码/Docker 执行是预期的本地开发路径。
...
### 📺完整设置指南 - Router + Claude Code 免费
  
[![Router + Claude Code Setup](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)
...
- 您的 CLI 工具发送 OpenAI 格式 → Router 转换 → 提供商接收原生格式
...
> Router 本身**从不向您收费**。您只需直接向提供商付款（如果使用付费服务）。
>
...
仪表板追踪您的 Token 使用情况，并显示**估算成本**，就像您直接使用付费 API 一样。这**不是实际计费** - 它是一个参考，显示您通过 Router 使用免费模型或现有订阅节省了多少。
...
<details>
<summary><b>💳 Router 会向我收费吗？</b></summary>
...
- ❌ **Router 本身** → **从不收取任何费用，永远**

Router 是本地代理/路由器。它没有您的信用卡，不能发送发票，也没有计费系统。完全免费的软件。
...
OpenAI API Key: [from router dashboard]
...
```json
{
  ...
  "anthropic_api_key": "your-router-api-key"
}
````
...
```bash
# Clone and install
git clone https://github.com/decolua/router.git
cd router
npm install
npm run build
...
export NEXT_PUBLIC_CLOUD_URL="https://router.com"
...
# Or use PM2
npm install -g pm2
pm2 start npm --name router -- start
pm2 save
pm2 startup
```
...
docker run -d \
  --name router  -p 20128:20128 \
  --env-file /root/dev/router/.env \
  -v router-data:/app/data \
  -v router-usage:/root/.router \
  router
```
...
| 变量                                                 | 默认值                            | 描述                                                                 |
| ---------------------------------------------------- | --------------------------------- | -------------------------------------------------------------------- |
| `JWT_SECRET`                                         | `router-default-secret-change-me` | 仪表板认证 cookie 的 JWT 签名密钥（**生产环境中请更改**）            |
| `INITIAL_PASSWORD                                    | `123456`                          | 当没有保存的哈希时的首次登录密码                                     |
| `DATA_DIR`                                           | `~/.router`                       | 主应用数据库位置（`db.json`）                                        |
...
| `CLOUD_URL`                                          | `https://router.com`              | 服务器端云同步端点基础 URL                                           |
| `NEXT_PUBLIC_BASE_URL`                               | `http://localhost:3000`           | 向后兼容/公共基础 URL（服务器运行时优先使用 `BASE_URL`）             |
| `NEXT_PUBLIC_CLOUD_URL`                              | `https://router.com`              | 向后兼容/公共云 URL（服务器运行时优先使用 `CLOUD_URL`）              |
...
- 使用历史和日志：`~/.router/usage.json` 和 `~/.router/log.txt`，由 `src/lib/usageDb.js` 管理。
...
- 验证 `CLOUD_URL` 指向您预期的云端端点（例如：`https://router.com`）
- 尽可能保持 `NEXT_PUBLIC_*` 值与服务器端值一致。

**云端端点 `stream=false` 返回 500（`Unexpected token 'd'...`）**

- 症状通常出现在公共云端端点（`https://router.com/v1`）的非流式调用上。
- 根本原因：上游返回 SSE 负载（`data: ...`）而客户端期望 JSON。
- 变通方法：对云端直接调用使用 `stream=true`。
- 当上游返回 `text/event-stream` 时，本地 Router 运行时包含 SSE→JSON 回退用于非流式调用。
...
- `tester/security/test-cloud-openai-compatible.sh`
  - 使用提供的模型/密钥向云端端点（`https://router.com/v1/chat/completions`）发送直接的 OpenAI 兼容请求。
...
- 公共云端端点（`https://router.com/v1/chat/completions`）：
...
- **网站**：[router.com](https://router.com)
- **GitHub**：[github.com/decolua/router](https://github.com/decolua/router)
- **问题**：[github.com/decolua/router/issues](https://github.com/decolua/router/issues)

---

## 👥 贡献者

感谢所有帮助让 Router 变得更好的贡献者！

[![Contributors](https://contrib.rocks/image?repo=decolua/router&max=100&columns=20&anon=1)](https://github.com/decolua/router/graphs/contributors)

---

## 📊 Star 图表

[![Star Chart](https://starchart.cc/decolua/router.svg?variant=adaptive)](https://starchart.cc/gzzeason/router)
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Router 的全功能 TypeScript 分支。添加了 36+ 提供商、4 层自动回退、多模态 API（图像、嵌入、音频、TTS）、熔断器、语义缓存、LLM 评估和精美的仪表板。8+ 单元测试。通过 npm 和 Docker 可用。
